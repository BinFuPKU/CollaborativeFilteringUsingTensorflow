...
def worker(fold, n_users, n_items, dataset_dir):
    ...
    gbprmf = GBPRMF(n_users, n_items, topN, rho, gsize, split_method, eval_metrics, reg, n_factors, batch_size)
    scores = gbprmf.train(fold+1, trasR, tstsR, sampler)

    print(dataset_dir.split('/')[-2] + '@%d:' % (fold + 1),'gsize=',gsize,'rho=',rho,'reg=',reg)
...
if __name__ =='__main__':
    print('gsize=',gsize,'rho=',rho,'reg=',reg)
