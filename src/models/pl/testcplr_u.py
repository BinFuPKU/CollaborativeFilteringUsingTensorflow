...
def worker(fold, n_users, n_items, dataset_dir):
    ...
    cplr = CPLR(n_users, n_items, topK, topN, split_method, eval_metrics,
                  alpha, beta, gamma,
                  reg, n_factors, batch_size)

    scores = cplr.train(fold+1, trasR, tstsR)
...
if __name__ =='__main__':
    ...
    results = []
    for fold in [2,3,4]: # range(folds_)
