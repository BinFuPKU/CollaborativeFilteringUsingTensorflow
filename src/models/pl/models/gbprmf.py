...
class GBPRMF(object):
    def __init__(self, n_users, n_items, topN=10, rho=.5, gsize=2,
                 ...
                 device='CPU'):
        # parameters
        self.__n_users, self.__n_items, self.__topN, self.__rho, self.__gsize = n_users, n_items, topN, rho, gsize
        ...
        self.__group_placeholder = tf.placeholder(tf.int32, shape=[None, None])
    ...
    def __reg_loss__(self):
        ...
        reg_loss_ += tf.nn.l2_loss(tf.nn.embedding_lookup(self.__user_embed, self.__group_placeholder)) # self.__rho *
        reg_loss_ += tf.nn.l2_loss(tf.nn.embedding_lookup(self.__item_embed, self.__useritem_placeholder[:, 1]))
        reg_loss_ += tf.nn.l2_loss(tf.nn.embedding_lookup(self.__item_bias,  self.__negItems_placeholder))
        return  self.__reg * reg_loss_

    @property
    def __embed_loss__(self):
        # user embedding (N, K)
        u = tf.nn.embedding_lookup(self.__user_embed, self.__useritem_placeholder[:, 0], name="u_embed_")
        # positive item embedding (N, K)
        i = tf.nn.embedding_lookup(self.__item_embed, self.__useritem_placeholder[:, 1], name="i_embed_")
        # negative items embedding (N, x, K)
        js = tf.nn.embedding_lookup(self.__item_embed, self.__negItems_placeholder, name="js_embed_")
        # group users embeddings (N, gsize, K)
        g = tf.nn.embedding_lookup(self.__user_embed, self.__group_placeholder, name="g_embed_")

        # positive item bias (N)
        i_bias = tf.nn.embedding_lookup(self.__item_bias, self.__useritem_placeholder[:, 1], name="i_bias_")
        # negative items bias (N, x)
        js_bias = tf.nn.embedding_lookup(self.__item_bias, self.__negItems_placeholder, name="js_bias_")

        ui_u = tf.reduce_sum(u * i, reduction_indices=-1, name="ui_u_posR_")
        ui_g = tf.divide(tf.reduce_sum(g * tf.expand_dims(i, axis=1), reduction_indices=[1,2]),
                         tf.cast(self.__gsize, tf.float32), name="ui_g_posR_")
        ui = tf.add(self.__rho * ui_g + (1-self.__rho) * ui_u, i_bias, name="ui_posR_")

        uj = tf.add(tf.reduce_sum(tf.expand_dims(u, 1) * js, reduction_indices=-1), js_bias, name="uj_negR_")
        ...
        with tf.device(self.__DEVICES[0]):
            ...
            for iter in range(self.__max_iter):
                ...
                for _ in range(n_batches):
                    useritem_pairs, negItems, group  = self.__sampler.next_batch()
                    _, loss = self.__sess.run(train_op, {self.__useritem_placeholder: useritem_pairs,
                                                         self.__group_placeholder:group,
                                                         self.__negItems_placeholder:negItems})
