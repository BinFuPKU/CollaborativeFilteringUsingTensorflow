...
class Ensemble(object):
    def __init__(self, n_users, n_items,
        ...
        # property
        self.__reg_loss__
        self.__ensemble_embed_loss__
        self.__singles_embed_loss__
        self.__loss
        self.__predict__
        self.__optimize__
    ...
    def __reg_k_loss__(self, k):
        reg_k_loss_ = tf.nn.l2_loss(tf.nn.embedding_lookup(self.__user_embeds[k,:,:], self.__useritem_placeholder[:, 0]))
        reg_k_loss_ += tf.nn.l2_loss(tf.nn.embedding_lookup(self.__item_embeds[k,:,:], self.__useritem_placeholder[:, 1]))
        reg_k_loss_ += tf.nn.l2_loss(tf.nn.embedding_lookup(self.__item_embeds[k,:,:], self.__negItems_placeholder))
        return reg_k_loss_
    ...
    def __ensemble_embed_score_a__(self, k):
        # user embedding (N, K)
        user_embed = tf.nn.embedding_lookup(self.__user_embeds[k,:,:], self.__useritem_placeholder[:, 0])
        # positive item embedding (N, K)
        posItem_embed = tf.nn.embedding_lookup(self.__item_embeds[k,:,:], self.__useritem_placeholder[:, 1])
        # negative item embedding (N, w, K)
        negItems_embed = tf.nn.embedding_lookup(self.__item_embeds[k,:,:], self.__negItems_placeholder)

        # n * k
        ui = user_embed * posItem_embed
        # n * w * k
        ujs = tf.expand_dims(user_embed, 1) * negItems_embed

        # n * 1
        ui_a = tf.reduce_sum(tf.exp(tf.matmul(ui, tf.expand_dims(tf.transpose(self.__h_embeds[k,:]), 1))), reduction_indices=-1)
        # n * w
        ujs_a = tf.exp(tf.reduce_sum(ujs * tf.expand_dims(self.__h_embeds[k,:], 0), -1))

        # n
        ui_rating = tf.reduce_sum(ui, reduction_indices=-1) * ui_a
        # n * w
        ujs_rating = tf.reduce_sum(ujs, reduction_indices=-1) * ujs_a

        return ui_rating, ujs_rating, ui_a, ujs_a


    @property
    def __ensemble_embed_loss__(self):
        score_a = []
        ui_a_base, ujs_a_base = 0, 0
        for i in range(self.kensemble):
            x1, x2, x3, x4 = self.__ensemble_embed_score_a__(i)
            score_a.append([x1, x2, x3, x4])
            ui_a_base += score_a[-1][-2]
            ujs_a_base += score_a[-1][-1]
        ui_rating, ujs_rating = 0, 0
        for i in range(self.kensemble):
            ui_rating += score_a[i][0] / ui_a_base
            ujs_rating += score_a[i][1] / ujs_a_base
        ensemble_embed_loss_ = tf.reduce_sum( - tf.log(tf.sigmoid(tf.expand_dims(ui_rating, 1) - ujs_rating)), name="ensemble_embed_loss_")

        return ensemble_embed_loss_

    def __single_embed_loss__(self, k):
        # user embedding (N, K)
        user_embed = tf.nn.embedding_lookup(self.__user_embeds[k,:,:], self.__useritem_placeholder[:, 0])
        # positive item embedding (N, K)
        posItem_embed = tf.nn.embedding_lookup(self.__item_embeds[k,:,:], self.__useritem_placeholder[:, 1])
        ...
        # n * w * k
        ujs = tf.reduce_sum(tf.expand_dims(user_embed, 1) * negItems_embed, reduction_indices=-1)

        single_embed_loss_ = tf.reduce_sum(-tf.log(tf.sigmoid(tf.expand_dims(ui,-1)- ujs)), name="embed_loss_")
        return single_embed_loss_


    @property
    def __singles_embed_loss__(self,):
        singles_embed_loss = 0
        for i in range(self.kensemble):
            singles_embed_loss += self.__single_embed_loss__(i)
        return singles_embed_loss

    @property
    def __loss(self):
        return self.__singles_embed_loss__ + self.ensemble_lambda * self.__ensemble_embed_loss__ \
               + self.__reg *  self.__reg_loss__
        ...
        with tf.device(self.__DEVICES[0]):
            ...
            for iter in range(self.__max_iter):
                ...
                for _ in range(n_batches):
                    ...
                    # print(useritem_pairs_batch.shape, negItems_batch.shape)
